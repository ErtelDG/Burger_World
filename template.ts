...
function renderMenuListTemplate(
   i: number,
   nameMenu: string,
   ingredientsMenu: string,
   priceMenu: number
) {
   return `
...
function renderMenuesWhereAreInTheBasketTemplate(
   basketAmount: number[],
   basketMenu: string[],
   totalPriceOneMenu: string,
   i: number
) {
   return `
...
function HowMuchIsMissingForFreeDeliveryTemplate(subtotal: number) {
   return `
